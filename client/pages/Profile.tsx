...
import { Edit2, Save, X, Camera, Download, Printer, Play, Pause } from "lucide-react";
...
export default function Profile() {
  ...
  // --- LOGIKA MUSIK DJ & ANIMASI ---
  const [isPlaying, setIsPlaying] = useState(false);
  const audioRef = useRef<HTMLAudioElement | null>(null);

  // GANTI LINK LAGU DI SINI
          ...
          <div className="bg-card border border-border rounded-xl p-8 shadow-lg">
            <div className="space-y-6">
              ...
              <div className="flex gap-4 pt-6 border-t border-border">
                <button onClick={handleSave} className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-lg font-semibold hover:opacity-90"><Save className="w-5 h-5" /> Simpan Perubahan</button>
                <button onClick={handleCancel} className="flex items-center gap-2 px-6 py-3 bg-muted text-foreground rounded-lg font-semibold hover:bg-muted/80"><X className="w-5 h-5" /> Batal</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto animate-slide-in-left">
        <div className="mb-8 flex justify-between items-start">
          ...
          <div className="flex gap-3">
            ...
            {isAdmin && <button onClick={handleEdit} className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-lg font-semibold hover:opacity-90 transition-all shadow-lg"><Edit2 className="w-5 h-5" /> Edit</button>}
          </div>
        </div>

        <div className="bg-card border border-border rounded-xl p-8 shadow-lg mb-6">
          <div className="flex flex-col md:flex-row gap-8 items-start">
            {/* AVATAR DENGAN ANIMASI DJ SET */}
            <div className="flex-shrink-0 relative group">
              {/* Border Animasi (Hanya muncul saat Play) */}
              <div className={`absolute -inset-2 rounded-2xl bg-gradient-to-r from-cyan-400 via-purple-500 to-pink-500 opacity-75 blur transition duration-1000 ${isPlaying ? 'animate-spin-slow scale-110' : 'opacity-0 scale-100'}`}></div>
              
              <div 
                onClick={handleToggleMusic}
                className="relative w-32 h-32 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg cursor-pointer overflow-hidden group"
              >
                {/* Overlay Ikon Play/Pause */}
                <div className={`absolute inset-0 z-10 flex items-center justify-center bg-black/20 transition-opacity duration-300 ${isPlaying ? 'opacity-0 hover:opacity-100' : 'opacity-100'}`}>
                  {isPlaying ? (
                ...
                {/* Inisial Nama */}
                <span className={`text-white text-5xl font-bold font-poppins transition-transform duration-500 ${isPlaying ? 'scale-75' : 'scale-100'}`}>
                  {profile.name.charAt(0)}
                </span>

                {/* Visualizer Bar (Hanya muncul saat Play) */}
                {isPlaying && (
                  <div className="absolute bottom-1 flex gap-0.5 h-6 items-end">
                    {[1, 2, 3, 4].map((i) => (
                      <div key={i} className="w-1.5 bg-white/80 animate-bounce" style={{ animationDelay: `${i * 0.1}s`, height: `${Math.random() * 100}%` }}></div>
                    ))}
                  </div>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-card border border-border rounded-xl p-6 shadow-lg">
            ...
            <div className="space-y-3">
              <p className="text-sm"><span className="text-foreground/70">Program Studi:</span> <span className="font-semibold">{profile.major}</span></p>
              <p className="text-sm"><span className="text-foreground/70">Tahun:</span> <span className="font-semibold">{profile.year}</span></p>
            </div>
          </div>
          <div className="bg-card border border-border rounded-xl p-6 shadow-lg">
            <h3 className="text-lg font-semibold text-foreground mb-4 pb-3 border-b border-border">Magang</h3>
            <div className="space-y-3">
              <p className="text-sm"><span className="text-foreground/70">Perusahaan:</span> <span className="font-semibold">{profile.companyName}</span></p>
              <p className="text-sm"><span className="text-foreground/70">Posisi:</span> <span className="font-semibold">{profile.position}</span></p>
            </div>
          </div>
        </div>

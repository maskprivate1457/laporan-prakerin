...
export default function Profile() {
  ...
  // Sinkronisasi auto-update di tab lain
  useEffect(() => {
        ...
        <div className="max-w-4xl mx-auto animate-slide-in-left">
          ...
          <div className="bg-card border border-border rounded-xl p-8 shadow-lg">
            <div className="space-y-6">
                ...
                <div className="flex flex-col md:flex-row gap-6 items-center">
                  ...
                  <div className="flex-1 w-full">
                    <label className="flex items-center gap-2 text-sm font-medium text-foreground mb-2">
                      <LinkIcon className="w-4 h-4" /> Atau Tempel URL Gambar
                    </label>
                    <input
                      type="text"
                      name="avatar"
                    ...
                    />
                    <p className="text-xs text-foreground/50 mt-2 italic">* Mendukung format file (PNG, JPG) atau link langsung dari internet.</p>
                  </div>
                </div>
              </div>
              ...
              {/* Seluruh field lainnya tetap sama prinsipnya */}
              {/* Misal school, major, year, internshipPeriod, supervisor, companyName, position, supervisor1, emailcompany, emailSchool, instagram, bio */}
              {/* Gunakan input type="text" atau textarea untuk bio */}
              {/* Semua menggunakan name yang sama dengan properti StudentProfile */}
              {/* Semua memiliki onChange={handleInputChange} */}
              {/* Klik Simpan Perubahan → handleSave → auto-sync di localStorage dan preview */}
  ...
  return (
    <Layout>
      <div className="max-w-4xl mx-auto animate-slide-in-left">
        ...
        <div className="mb-8 flex justify-between items-start">
          ...
          <div className="flex gap-3">
            <button onClick={handleDownload} className="flex items-center gap-2 px-6 py-3 bg-secondary text-secondary-foreground rounded-lg font-semibold shadow-lg">
              <Download className="w-5 h-5" /> Download
            </button>
            <button onClick={handlePrint} className="flex items-center gap-2 px-6 py-3 bg-accent text-accent-foreground rounded-lg font-semibold shadow-lg">
              <Printer className="w-5 h-5" /> Print
            </button>
            {isAdmin && <button onClick={handleEdit} className="flex items-center gap-2 px-6 py-3 bg-primary text-primary-foreground rounded-lg font-semibold shadow-lg">
        ...
        {/* Avatar Preview */}
        <div className="bg-card border border-border rounded-xl p-8 shadow-lg mb-6 flex flex-col md:flex-row gap-8 items-start">
          <div className="flex-shrink-0 relative group">
            ...
            <div onClick={toggleMusic} className={`relative w-32 h-32 rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-lg overflow-hidden cursor-pointer z-10 transition-all duration-300 ${isPlaying ? 'scale-105 shadow-2xl ring-2 ring-white/50' : 'scale-100 border border-border'}`}>
              {profile.avatar ? <img src={profile.avatar} alt={profile.name} className="w-full h-full object-cover" /> : <span className="text-white text-5xl font-bold">{profile.name.charAt(0)}</span>}
              <div className={`absolute inset-0 flex items-center justify-center bg-black/40 transition-all duration-300 ${isPlaying ? 'opacity-0 group-hover:opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                {isPlaying ? <Pause className="w-12 h-12 text-white animate-pulse" /> : <Play className="w-12 h-12 text-white" />}
              </div>
              {isPlaying && (
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div><p className="text-foreground/70">Telepon</p><p className="font-semibold text-foreground">{profile.phone}</p></div>
              <div><p className="text-foreground/70">Sekolah</p><p className="font-semibold text-foreground">{profile.school}</p></div>
            </div>
          </div>
        </div>

        {/* ... Sisa field preview seperti Akademik, Magang, Biodata tetap sama ... */}
      ...
      <style jsx>{`
        ...
        @keyframes bar-bounce {0%,100%{height:20%;}50%{height:80%;}}
